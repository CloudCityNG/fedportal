...
class AssessmentController
{
   ...
   */
  private static $SCORE_GRADE_MAPPING = [
    [70, 100, 'A'],
  ...
  public static function post()
  {
    if (isset($_POST['reg-no-submit'])) {
      ...
      $postedCourses = StudentCourses::getStudentCurrentCourses([
        'reg_no' => $regNo,
        'semester_id' => $oldStudentCourseQueryData['semester_id']
      ]);

      self::renderPage(null, null, ['courses' => $postedCourses]);
      ...
      /**
       * all data about student, including his courses that we sent to the client. This is posted back from client
       * to server so we can re-render the details in case of post success or failure
       *
      ...
      /**
       * @var $updatedCourses - courses that we are able to update with student's grades. Note that it is possible that not all
      ...
      /**
       * @var $studentCourses - student data that were sent back from client - we extract only the courses
       */
      ...
      if ($countUpdatedCourses) {
        ...
        /**
         * @var array $successfullyUpdatedCourses - courses that were successfully updated by database.
         * The code that performed the database update will
         * only send course id and score back to us. We extract other course data from @var $studentCourses
         */
  ...
  )
  {
    ...
    $scoreGradeMapping = self::$SCORE_GRADE_MAPPING;
